...
public static class GisHelper
{
    ...
    public static STRtree<TEntity> CreateGenericIndex<TEntity>(IEnumerable<TEntity> entities) where TEntity : IGisEntity
    {
        var tree = new STRtree<TEntity>();
        foreach (var entity in entities)
        {
            if (entity?.Geometry != null)
            {
               
                tree.Insert(entity.Geometry.EnvelopeInternal, entity);
            }
        }
        tree.Build();
        return tree;
